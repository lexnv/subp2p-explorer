...
impl<'a> From<LiteP2pProtocol<'a>> for Protocol<'a> {
    fn from(protocol: LiteP2pProtocol<'a>) -> Self {
        match protocol {
            ...
            protocol => {
                log::error!("Got unsupported multiaddr protocol '{}'", protocol.tag(),);
...
impl<'a> From<Protocol<'a>> for LiteP2pProtocol<'a> {
    fn from(protocol: Protocol<'a>) -> Self {
        match protocol {
            ...
            // Protocol #280 was called `WebRTC` in multiaddr-17.0 and is
            // `WebRTCDirect` since multiaddr-18.1.
            ...
            Protocol::P2p(multihash) => LiteP2pProtocol::P2p(
                PeerId::from_multihash(multihash.into()).unwrap_or_else(|_| {
